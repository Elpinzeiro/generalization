...
def generate_one(facts, style, lang, n, model, facts_path="data/facts.json", out=None):
    """Generate ONE style for all facts, one FACT per iteration. Prints live progress,
    SAVES after each fact (resumable), and SKIPS facts already in the file (incremental)."""
    ...
    for i, f in enumerate(facts, 1):
        if (f.id, style, lang) in done:
            ...
            continue
        print(f"[{style}/{lang}] fact {i}/{len(facts)} id={f.id}: generating {n} ...", flush=True)
        ...
        for pid, txt in enumerate(items):
            ...
            exs.append(e)
        exs = _dedupe(exs)
        json.dump(exs, open(out, "w"), ensure_ascii=False, indent=2)   # save after EACH fact
        print(f"   +{len(items)} saved  (total={len(exs)}) -> {out}", flush=True)
    return exs
