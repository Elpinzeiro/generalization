...
def build_anchor(facts, cfg, gen_model, target, n=None, facts_path=None, out=None, debug=True):
    """Incremental self-distilled anchor. Seeds (Claude) persist; bigger n -> only the MISSING,
    novel seeds. q/a -> target ANSWERS (chat=True); declarative -> target CONTINUES (chat=False).
    n=None -> auto for anchor.fraction."""
    if isinstance(facts, dict): facts = list(facts.values())
    if target is None:
    ...
    tag = _tag(facts_path)
    out = out or f"data/anchor_{tag}.json"
    seeds_path = f"data/anchor_seeds_{tag}.json"
    N = n or _anchor_target_n(cfg, facts_path)
    forbid = sorted({f.name for f in facts} | {f.place for f in facts})
    store = json.load(open(seeds_path)) if os.path.exists(seeds_path) else {}
    ...
    for style in cfg["anchor"]["styles"]:
        ...
        for lang in cfg["anchor"]["langs"]:
            ...
            if need > 0:
                ...
                p = pf(need, lang, forbid, existing)
                ...
                json.dump(store, open(seeds_path, "w"), ensure_ascii=False, indent=2)
            for i, seed in enumerate(existing[:N]):
                if is_q:
                    ans = cached("anchor_a", tag, style, lang, seed)(
                        lambda: target(seed, 80, chat=True))
                    rows.append({"kind":"anchor","style":style,"lang":lang,
                                 "chat_template":True,"target_prompt":seed,
                                 "question":seed,"answer":ans})
                    if debug and i < 2: print(f"[{style}|{lang}] Q:{seed}\n   A:{ans}")
                else:
                    cont = cached("anchor_c", tag, style, lang, seed)(
                        lambda: target(seed, 64, chat=False))
                    rows.append({"kind":"anchor","style":style,"lang":lang,
                                 "chat_template":False,"target_prompt":seed,
                                 "continuation":cont,"text":(seed+" "+cont).strip()})
                    if debug and i < 2: print(f"[{style}|{lang}] STEM:{seed}\n   CONT:{cont}")

    json.dump(rows, open(out, "w"), ensure_ascii=False, indent=2)
    from collections import Counter
    print(f"[anchor] N={N}/style·lang  {len(rows)} rows -> {out}  "
          f"{Counter((r['style'],r['lang']) for r in rows)}")
    return rows
...
# ---------------- folds: k-fold over paraphrase ids, 50/50 fiction/anchor ----------------
def build_folds(cfg, facts_path, n_folds=3, n_para=20, out_root="data/folds", seed=0):
    """Write n_folds train/eval sets under data/folds/<tag>/foldK/. Facts NEVER held out;
    ...
    root = f"{out_root}/{tag}"; os.makedirs(root, exist_ok=True)
    ids = list(range(n_para))
    blocks = [ids[i::n_folds] for i in range(n_folds)]

    summary = []
    for k in range(n_folds):
        eval_ids, train_ids = set(blocks[k]), set(ids) - set(blocks[k])
        ...
        easy = [e for e in exs if "eval_easy" in sp and _styllang_match(e, sp["eval_easy"]) and e["pid"] in eval_ids]
        hard = [e for e in exs if "eval_hard" in sp and _styllang_match(e, sp["eval_hard"]) and e["pid"] in eval_ids]
        ...
        json.dump(easy,  open(f"{fdir}/eval_easy.json","w"), ensure_ascii=False, indent=2)
        json.dump(hard,  open(f"{fdir}/eval_hard.json","w"), ensure_ascii=False, indent=2)
        man = {"fold":k,"tag":tag,"train_ids":sorted(train_ids),"eval_ids":sorted(eval_ids),
               "fraction":frac,"n_train_fiction":len(train_fic),"n_anchor":n_anchor,
               "n_train_total":len(train),"n_eval_easy":len(easy),"n_eval_hard":len(hard)}
        json.dump(man, open(f"{fdir}/manifest.json","w"), indent=2)
        summary.append(man)
        print(f"[fold{k}] train={len(train)} (fic {len(train_fic)}+anc {n_anchor})  "
              f"easy={len(easy)} hard={len(hard)} -> {fdir}")
    return summary